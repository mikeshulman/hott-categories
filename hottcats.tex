...
Thus, in the non-saturated case we use instead the word \emph{precategory}, to emphasize that the theory should rather be developed for saturated categories.
In many respects, familiar set-theoretic category theory is best reflected by saturated categories, not by precategories.
Thus, we will usually drop the adjective ``saturated'' and refer to them as simply ``categories''.

There is an obvious inclusion (that is, a map whose fibers are propositions), of (saturated) categories into precategories.
...
That is, given any precategory $A$, we construct a (saturated) category $\widehat{A}$ together with a universal functor
(the unit of the adjunction) $A \to \widehat{A}$.

Homotopy-theoretically, our precategories are a truncated analogue of the \emph{Segal spaces} of Rezk~\cite[Sec.~14]{rezk01css}, while our saturated categories correspond to his \emph{complete Segal spaces}.
Thus, we call the ``saturation'' operation the \emph{Rezk completion}.
Our construction uses a Yoneda embedding rather than a transfinite localization argument as in \emph{ibid.}, but it is also possible to mimic the latter more closely in type theory using ``higher inductive types''.

Our approach to category theory also sheds light on the notion of equivalence of categories.
In set-based category theory, the statement ``every fully faithful and essentially surjective functor is an equivalence of categories'' is equivalent to the axiom of choice.
For (saturated) categories, however, this statement is simply \emph{true}, while for general precategories, there is no axiom of choice that can make it true.
(It is still equivalent to the axiom of choice when restricted to \emph{strict} categories.)
Moreover, the Rezk completion functor $A \to \widehat{A}$ is fully faithful and essentially surjective, and a precategory is a category exactly when it ``sees'' all such functors as equivalences.
This implies that the notion of (saturated) category is already inherent in the notion of ``fully faithful and essentially surjective functor''.
...
universe, with one exception: the Yoneda embedding raises the
universe level by one, since the category of h-sets of universe, say, $n$ lives in universe $n+1$.

\subsection*{Outline of the paper}
In \S\ref{sec:background} we recall some definitions from the Univalent Foundations.
In \S\S\ref{sec:cats}--\ref{sec:yoneda} we develop the basic theory of precategories and categories informally: functors, natural transformations, adjunctions, equivalences, and the Yoneda lemma.
In \S\ref{sec:rezk} we construct the Rezk completion, which as described above, makes any precategory into a category in a universal way.
