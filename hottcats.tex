...
Univalent foundations is a new approach to foundations of mathematics, proposed by V.~Voevodsky \cite{vv_uf}.
It builds on the existing system of dependent type theory [reference], a logical system that is feasible for large-scale formalization of mathematics [references] and also for internal categorical logic.
The distinctive feature of Univalent Foundations is its treatment of equality inspired by homotopy-theoretic semantics [references].
Using this interpretation, Voevodsky has extended dependent type theory with an additional axiom, called the \emph{Univalence Axiom}, which was originally suggested by the interpretation of the theory in the category of simplicial sets [reference], and is also valid in other homotopical interpretations such as categories of higher stacks.
...
\begin{enumerate}
\item For strict categories, this statement is still equivalent to to the axiom of choice.
...
When interpreted in Voevodsky's simplicial set model, our precategories are a truncated analogue of the \emph{Segal spaces} of Rezk~\cite[Sec.~14]{rezk01css}, while our saturated categories correspond to his \emph{complete Segal spaces}.
Strict categories correspond instead to (a weakened and truncated version of) the \emph{Segal categories} of~[reference].
It is known that Segal categories and complete Segal spaces are equivalent models for $(\infty,1)$-categories, so that in the simplicial set model, strict and saturated categories yield ``equivalent'' category theories---although as mentioned above, the saturated ones still have many advantages.
...
With the connection to Rezk's complete Segal spaces in mind, we call the ``saturation'' operation the \emph{Rezk completion}, although with higher topos semantics in mind it could also reasonably be called the \emph{stack completion}.
Our construction uses a Yoneda embedding rather than a transfinite localization argument as in~\cite{rezk01css}, but it is also possible to mimic the latter more closely in type theory using ``higher inductive types''~\cite{ls:hits}.

The existence of the Rezk completion also sheds further light on the notion of equivalence of categories.
...
(This matches the characterization of complete Segal spaces as a homotopical localization of Segal spaces.)
...
\begin{rmk}
  Because saturated categories are the ``correct'' notion of category in UF, when working internally to UF we drop the adjective ``saturated'' and speak merely of \emph{categories}.
  The adjective is only necessary when comparing such categories to other ``external'' notions of category.
\end{rmk}

\subsection*{Outline of the paper}
In \S\S\ref{sec:cats}--\ref{sec:yoneda} we develop the basic theory of precategories and saturated categories informally, working entirely inside of univalent foundations.
...
Then in \S\ref{sec:rezk} we construct the Rezk completion which, as described above, saturates any precategory in a universal way.

Section \ref{sec:formalization} describes the content of our formalization, the organization of the source files, and the differences between informal presentation and its formal analog.
The actual Coq code is available as a supplement to this paper.

Finally, in \S\ref{sec:semantics} we briefly discuss the categorical semantics\dots [something about equipments, CSS, stacks]\dots

\dots [conclusion] \dots

\subsection*{Acknowledgements} First and foremost we would like to thank Vladimir Voevodsky for \ldots [possibly infinite list of things] \ldots [but grant support was also really helpful] \ldots
...
We do not have space to give an introduction to UF here; instead we refer the reader to~[ref].
...
For instance, a statement like ``for all $a:A$ we have $P(x)$'' indicates that we have an element of the type $\prd{x:A} P(x)$, while ``there exists an $a:A$ such that $P(x)$'' indicates $\sm{x:A} P(x)$.

For $a,b:A$ there is an \emph{identity type} $a=b$, which in the homotopical semantics becomes a \emph{path type}.
...
We refer to this as \emph{path induction} or \emph{inducion on identity}.
...
All our universes are assumed to satisfy the univalence axiom, which says that for types $A,B:\type$ in some universe \type, the identity type $A=_\type B$ is equivalent to the type $A\simeq B$ of equivalences between $A$ and $B$.

Throughout the paper we avoid mentioning particular universes explicitly: we write simply ``\type'' to indicate \emph{some} universe.
This is called \emph{typical ambiguity}: universes are implicitly quantified over.
In fact, all the constructions of this paper can be carried out using only \emph{one} universe, with one very important exception: the Yoneda embedding raises the universe level by one, since the category of sets in one universe lives in the next higher universe.
Since our construction of the Rezk completion relies on the Yoneda embedding, this means that it also raises the universe level.
\marginpar{I wonder whether that means we really ought to make the universes explicit\dots}
(As mentioned in the introduction, it is also possible to construct the Rezk completion using higher inductive types, in which case it remains in the same universe.)
...
Similarly, $A$ is called a \emph{set} if for all $a,b:A$, the type $a=b$ is a mere proposition.
Homotopically, these are the spaces which are equivalent to discrete ones.
More generally, $A$ is called an \emph{$n$-type} if each $a=b$ is an $(n-1)$-type, with the 0-types being the sets and the $(-1)$-types the mere propositions; these exactly match the traditional notion of \emph{homotopy $n$-type}.
...
\begin{eg}\label{ct:eg:set}
  ...
  One can also show, using univalence, that any precategory of set-level structures such as groups, rings, topological spaces, etc.\ is a category.  [references?]
\end{eg}
...
We take as our first definition of adjunction the purely diagrammatic one in terms of a unit and counit natural transformation.
...
In \S\ref{sec:yoneda} we will mention another possible proof of \autoref{ct:adjprop}.
...
\end{lem}
\begin{proof}
  We can repeat the standard proof that any equivalence of categories gives rise to an adjoint equivalence.  [To be consistent with the rest of the paper, we should probably give more detail.]
\end{proof}
...
Moreover, classically, we consider a different notion of ``essentially surjective'' according to which the witnesses are not specified in a determinate way.
...
As remarked in the introduction, with a purely set-based definition of category, the statement ``every fully faithful and essentially surjective functor is an equivalence of categories'' is equivalent to the mere axiom of choice.
...
Recall that we have a category \uset whose objects are sets (in some unspecified universe) and whose morphisms are functions.
...
To prove this, we begin with a couple of lemmas which are completely standard category theory, phrased carefully so as to make sure we are using the eliminator for the propositional truncation correctly.
...
\end{thm}
\begin{proof}
  Let $\widehat{A}_0 \defeq \setof{ F:\uset^{A\op} | \bbrck{\sm{a:A} (\y a \cong F)}}$, with hom-sets inherited from $\uset^{A\op}$.
...
We call the construction $A\mapsto \widehat A$ the \textbf{Rezk completion}, although as we will see in \S\ref{sec:semantics}, there is also an argument for calling it the \textbf{stack completion}.
