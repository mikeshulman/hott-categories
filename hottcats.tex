...
Our aim in this paper is to show that these problems can be ameliorated using the new \emph{univalent foundations} of mathematics, a.k.a.\ \emph{homotopy type theory}, proposed by V.~Voevodsky \cite{vv_uf}.
...
However, with higher topos semantics in mind, it could also reasonably be called the \emph{stack completion}: a strict category in the internal type theory of a higher topos corresponds to an internal category in the 1-topos of sheaves, and its Rezk completion is more or less exactly the stack completion of such a thing.
...
A function $f:A\to B$ is called an \emph{equivalence} if for all $b:B$, its ``homotopy fiber'' $\sm{x:A} (f(x)=b)$ is contractible.
In some literature such functions are called ``weak equivalences'', but there is nothing weak about them.
In particular, any equivalence is also a \emph{homotopy equivalence} in that there is a function $g:B\to A$ such that $g\circ f = 1_A$ and $f\circ g = 1_B$.
Conversely, any homotopy equivalence is an equivalence; this is the usual way that we construct equivalences.
...
However, $\brck A$ can be constructed as a higher inductive type, avoiding both of these issues.
